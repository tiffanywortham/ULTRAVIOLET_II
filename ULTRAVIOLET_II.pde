...
void draw() {
  ...
  if(STATE == 4) {
    ...
    if(STATE == 5) {
      ...
      f = createFont("Poor Richard", 48);
      textFont(f);
      text("You have survived the darkness!", 700, 400);
      if(keyPressed == true) {
        STATE = 4;
      }
  }
...
private void runGame() {
  ...
  if((cheese.found == true) && (bread.found == true) && (salami.found == true) && (lettuce.found == true) && (mayo.found == true) && (tomato.found == true) && (onion.found == true)) {
    STATE = 5;
  }
 ...
 // Loop through each pixel in the window
  for (int x = 0; x < width; x++ ) {
    for (int y = 0; y < height; y++ ) {
      ...
      float adjustBrightness = map(distance, 0, gameTimer/DIFF, 2, 0.3);
      r *= adjustBrightness;
      g *= adjustBrightness;
      b *= adjustBrightness;
